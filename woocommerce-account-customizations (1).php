...
function cd_process_vehicle_data_after_order_completion($order_id) {
    track_function_call('cd_process_vehicle_data_after_order_completion', [
        'order_id' => $order_id,
        'hook' => current_filter()
    ]);
...
function cd_import_vehicle_from_orders() {
    // Only proceed if user is on vehicles page and has no vehicles yet
    if (is_account_page() && is_wc_endpoint_url('vehicles')) {
        $user_id = get_current_user_id();
        ...
        if ($existing_count == 0) {
            ...
            foreach ($orders as $order) {
                ...
                if (get_post_meta($order_id, '_cd_vehicle_processed', true) === 'yes') {
                    continue;
                }
                ...
                if (!empty($reg) && !empty($make) && !empty($model) && !empty($color)) {
                    ...
                    if (!$exists) {
                        ...
                        update_post_meta($order_id, '_cd_vehicle_processed', 'yes');
                        
                        // Refresh the page after adding vehicle
...
function cd_store_vehicle_data_on_checkout($order_id) {
    ...
    // Check if we've already processed this order to prevent multiple submissions
    if (get_post_meta($order_id, '_cd_vehicle_processed', true) === 'yes') {
        return;
    }
    ...
    <script>
    (function() {
        // Only run once using a flag in localStorage
        if (localStorage.getItem('cd_form_submitted_<?php echo $order_id; ?>') === 'yes') {
            return;
        }
        ...
        if (registrationData) {
            ...
            localStorage.setItem('cd_form_submitted_<?php echo $order_id; ?>', 'yes');
...
function cd_process_vehicle_data_submission() {
    // Check if we have vehicle data to process
    if (isset($_POST['cd_vehicle_data']) && isset($_POST['cd_order_id'])) {
        $order_id = intval($_POST['cd_order_id']);
        $order = wc_get_order($order_id);
        
        // Check if we've already processed this order
        if ($order && get_post_meta($order_id, '_cd_vehicle_processed', true) !== 'yes') {
            ...
            if (is_array($vehicle_data)) {
                ...
                if ($user_id) {
                    ...
                    // Check if vehicle already exists before adding
                    global $wpdb;
                    $registration = strtoupper(sanitize_text_field($vehicle_data['car-reg']));
                    $exists = $wpdb->get_var($wpdb->prepare(
                        "SELECT id FROM {$wpdb->prefix}cd_vehicles 
                         WHERE user_id = %d AND registration_number = %s AND active = 1",
                        $user_id, $registration
                    ...
                    // Only add if vehicle doesn't exist
                    if (!$exists) {
                        $wpdb->insert(
                            $wpdb->prefix . 'cd_vehicles',
                            array(
                        ...
                        // Send notices to operators if needed
                        if ($wpdb->insert_id && function_exists('cd_get_email_manager')) {
                            $email_manager = cd_get_email_manager();
                            $email_manager->send_notices_to_all_operators($user_id, $wpdb->insert_id);
                        }
                    }
                }
                
                // Mark order as processed to prevent duplicate processing
                update_post_meta($order_id, '_cd_vehicle_processed', 'yes');
                
                // Redirect to the thank you page to break the loop
                if (wp_redirect(wc_get_endpoint_url('order-received', $order_id, wc_get_page_permalink('checkout')))) {
                    exit;
                }
            }
        }
